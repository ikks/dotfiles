...
def handle_result(args: list[str], answer: str, target_window_id: int, boss: Boss):
    ...
    for mode in opts.keyboard_modes.values():
        ...
        for key, action in key_mappings.items():
            ...
            key_repr = f"{key_repr:<22} {fmt.fg.red}→{fmt.fg.default}"
    ...
    output = [
        ...
        "",
        "ctrl+shift+f1          Opens local help",
        "ctrl+shift+f2          Edit kitty configuration",
        "ctrl+shift+f5          Reloads Configuration",
        "ctrl+shift+f6          Shows own keys and info",
        "",
    ]
    for category in categories:
        if category not in output_categorized:
            continue
        output.extend([f"## {fmt.bold}{category}{fmt.nobold}", ""])
        output.extend(sum(output_categorized[category].values(), []))
        output.append("")

    output.append(
        f"file://{HOME}/.local/kitty.app/share/doc/kitty/html/overview.html#other-keyboard-shortcuts"
    )

    with open(HOME + "/.local/share/local/kitty-help.txt", "w+") as f:
        f.write("\n".join(output))
